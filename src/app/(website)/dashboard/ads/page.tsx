    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link href={"/dashboard/ads/add-ads"}>
          <button className="bg-[#28a745] py-2 px-5 rounded-lg text-white font-semibold">+ Add Ads</button>
        </Link>
      </div>
      ...
      <div>
                ...
                <TableRow key={ad._id} className="border-b border-[#b0b0b0]">
                  ...
                  <TableCell className="text-center border-none">
                    <div className="flex justify-center">
                      <Switch checked={ad.publish} className="data-[state=checked]:bg-green-500" />
                    </div>
                  </TableCell>
                  <TableCell className="text-center border-none">{formatDate(ad.createdAt)}</TableCell>
                  <TableCell>
        ...
        <div className="flex items-center justify-between mt-4 text-sm">
          <div>
            Showing {indexOfFirstPost + 1}-{Math.min(indexOfLastPost, ads.length)} from {ads.length}
          </div>
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <Button
                  ...
                  size="icon"
                  className={`h-8 w-8 ${currentPage === pageNumber ? "bg-green-500" : ""}`}
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditFormData({ ...editFormData, adsTitle: e.target.value })}
                  required
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) => setEditFormData({ ...editFormData, adsContent: e.target.value })}
                  required
                  rows={4}
                />
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button variant="ghost" size="icon" onClick={() => setIsDeleteModalOpen(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            ...
            <div className="mb-6">
              <p className="text-gray-600">
                Are you sure you want to delete {adToDelete?.adsTitle}? This action cannot be undone.
              </p>
            </div>

            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={() => setIsDeleteModalOpen(false)} className="flex-1">
                Cancel
              </Button>
              <Button
