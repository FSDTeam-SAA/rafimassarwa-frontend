...
export default function Articles() {
  ...
  const { data: relatedDeepResearch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosInstance(`/admin/news/deep-research?symbol=${relatedStockParams}`);
      return res.data.data as DeepResearchItem[];
    },
  });

  console.log(deepResearch);
  console.log(relatedDeepResearch);
  ...
  const tabsData = [
    {
      value: "allstocks",
      title: "Market News",
      data: pathname === "/search-result" ? relatedStockNews || [] : stockNews || [],
    },
    {
      value: "deep-research",
      title: "Olive Stock's Deep Research",
      data: pathname === "/search-result" ? relatedDeepResearch || [] : deepResearch || [],
    },
  ];
